...
const Navigation = memo(() => {
  ...
  const navigationItems = useMemo(() => [
    ...
    ...(user ? [
      { path: '/customer-dashboard', label: 'Customer Portal' },
      { path: '/vendor-dashboard', label: 'Vendor Portal' }
    ] : [])
  ], [user]);

  return (
    <>
      <nav id="navigation" className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            {/* Logo with accessibility improvements */}
            <Link to="/" className="flex items-center space-x-2">
              <HardHat className="h-8 w-8 text-allrentz-red" aria-label="ALLRENTZ Logo" />
              ...
              {navigationItems.slice(0, 2).map(item => (
                <Link 
                  ...
                  className={`text-sm font-medium transition-colors ${
                    isActive(item.path) ? 'text-allrentz-red' : 'text-gray-700 hover:text-allrentz-red'
                  }`}
                >
                  {item.label}
                </Link>
            ...
            <button
              ...
              className="md:hidden p-2 rounded-md text-gray-700 hover:text-allrentz-red hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-allrentz-red focus:ring-offset-2"
