-- P1-3A Gate 1: Add DB-level transition allowlist enforcement to transition_rfq_status()
--
-- This migration adds a static allowlist check for valid (from_status → to_status)
-- pairs inside the existing SECURITY DEFINER function. The DB function is the
-- authoritative copy of the allowlist. Any Edge Function copy is an optimization only.
--
-- No signature change. No new tables. No new columns. No policy changes.
-- Depends on: 20260526150000_p1_operational_authority.sql
...
BEGIN
  ...
  IF NOT (v_transition_key = ANY(ARRAY[
    ...
    'in_transit:on_rent',
    'on_rent:rental_extended',
    'on_rent:off_rent_requested',
    'rental_extended:off_rent_requested',
